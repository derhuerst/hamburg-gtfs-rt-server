...
const debug = require('debug')('hamburg-gtfs-rt-server:match')
const pLimit = require('p-limit')
const {
	matchTrip: matchTripWithGtfs,
	matchMovement: matchMovementWithGtfs,
	createMatchTrip,
	createMatchMovement,
...
const matchTrip = async (trip) => {
	...
	await limit(async () => {
		...
		const isMatched = trip.ids && trip.ids['hvv-gtfs']
		if (isMatched) {
			matchedItems++
...
const matchMovement = async (mv) => {
	...
	await limit(async () => {
		...
		avgMatchTime.push(t2 - t1)
		avgWaitTime.push(t1 - t0)
		const isMatched = mv.tripIds && mv.tripIds['hvv-gtfs']
		if (isMatched) {
			matchedItems++
			matchedTripIds.add(origTripId)
		}
